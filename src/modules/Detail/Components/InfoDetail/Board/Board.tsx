...
function Board(props: IPropsFood) {
  ...
  const [cookTime, setCookTime] = useState<string>("10");
  const [totalTime, setTotalTime] = useState<string>("10");
  useEffect(() => {
    if (recipe) {
      setPrepTime(recipe?.[0]?.prepTime || "10");
      setCookTime(recipe?.[0]?.cookTime || "10");
      setTotalTime(recipe?.[0]?.totalTime || "10");
    }
  }, [recipe]);
  const handleDecreaseServes = () => {
    if (numberServes > 0) {
      setNumberServes(numberServes - 1);
      setPrepTime((prevPrepTime) => {
        const newPrepTime = parseInt(prevPrepTime) - 10;
        return newPrepTime >= 0 ? newPrepTime.toString() : "0";
      });
      setCookTime((prevCookTime) => {
        const newCookTime = parseInt(prevCookTime) - 10;
        return newCookTime >= 0 ? newCookTime.toString() : "0";
      });
      setTotalTime((prevTotalTime) => {
        const newTotalTime = parseInt(prevTotalTime) - 10;
        return newTotalTime >= 0 ? newTotalTime.toString() : "0";
      });
    }
  };
  const handleIncreaseServes = () => {
    setNumberServes(numberServes + 1);
    setPrepTime((parseInt(prepTime) + 10).toString());
    setCookTime((parseInt(cookTime) + 10).toString());
    setTotalTime((parseInt(totalTime) + 10).toString());
  };
  return (
    <div className="bg-[#f8f8f8] my-5" ref={componentRef}>
      ...
      <div className="flex flex-row items-center gap-5 justify-center mt-5">
        ...
        <div className=" cursor-pointer" onClick={handleDecreaseServes}>
          -
        </div>
        <div>{numberServes}</div>
        <div className="cursor-pointer" onClick={handleIncreaseServes}>
          +
        </div>
      </div>
      <div className="px-10 ">
          ...
          <div className="bg-white w-[500px] border-solid border-2 border-stone-800 flex flex-col items-start justify-start p-4">
            ...
            <div className="my-1">
              <p className="font-bold">
                Calories{" "}
                <span className="font-normal">{recipe?.[0]?.calories}</span>
              </p>
            </div>
