  ...
  return (
    <div className=" mx-auto px-2">
      ...
      <div className="flex flex-row gap-3 justify-between">
        {airFryer &&
          airFryer.map((fry, index) => (
            <Link to={`/detail/recipe/${fry.id}`} key={fry.id}>
              <div
                key={index}
                ...
                <div className="flex flex-col items-start justify-between bg-white hover:bg-orange-200 p-3">
                  ...
                  <div>
                    ...
                    <div className="font-semibold text-sm">
                      rating:{fry.rating}{" "}
                      <Rate disabled={true} value={fry.rating}></Rate>
                    </div>
                    <div className="mt-2 tracking-normal text-justify">
                      <p>{truncateIntro(fry.description, 5)}</p>
                    </div>
                  </div>
                </div>
              </div>
